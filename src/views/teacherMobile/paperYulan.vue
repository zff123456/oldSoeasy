<template>
  <div class="right" v-loading="isLoading" element-loading-text="拼命加载中">
    ...
    <div class="search-box">
      ...
      <el-button type="primary" icon="el-icon-search" @click="search" style="margin-left:.266667rem">搜索</el-button>
    </div>
    <!-- 表格筛选。 -->
    <div class="table-box">
    ...
    <!-- 分页插件。 -->
      <div style="float:right;padding-top:5px;height:40px;">
        <el-pagination
...
export default {
  ...
  methods: {
    ...
    async startKaoshi(e){ // 开始考试
      ...
      if (e.test_paper_type_id == 1){ this.$router.push({ name: "mobile-four-kaoshi",params: { isChakan: 0} },) };
      if (e.test_paper_type_id == 2){ this.$router.push({ name: "mobile-six-kaoshi",params: { isChakan: 0} }) };
      if (e.test_paper_type_id == 3){ this.$router.push({ name: "mobile-a-kaoshi",params: { isChakan: 0} }) };
      if (e.test_paper_type_id == 4){ this.$router.push({ name: "mobile-b-kaoshi",params: { isChakan: 0} }) };
    },
    async getList(){
