      ...
      <el-table :data="flatten(tableData.slice((currentPage -1) * pagesize, currentPage * pagesize))" stripe style="" border :row-style="{height:'48px'}"  :header-row-style="{height:'48px'}">
        ...
        <el-table-column label="操作" align="center" min-width="180">
          <template slot-scope="scope">
            <el-button size="mini" type="warning" :disabled="scope.row.state == 0" @click="qvxiaoYuyue(scope.row)">取消预约</el-button>
            <el-button size="mini" type="primary" :disabled="scope.row.state == 0" @click="startKaoshi(scope.row)">开始考试</el-button>
          </template>
        </el-table-column>
      </el-table>
      <!-- 分页插件。 -->
...
export default {
  ...
  methods: {
    ...
    async getList(){
      const res = await this.$http.post("admin/Examination/makeanappointmentlst", qs.stringify({ u_id: this.teacher_id}));
    ...
    async startKaoshi(e){ // 开始考试
      ...
      if (e.test_paper_type_id == 1){ this.$router.push({ name: "fourKaoshi" }) };
      if (e.test_paper_type_id == 2){ this.$router.push({ name: "sixKaoshi" }) };
      if (e.test_paper_type_id == 3){ this.$router.push({ name: "aKaoshi" }) };
      if (e.test_paper_type_id == 4){ this.$router.push({ name: "bKaoshi" }) };
    }
  }
};
</script>
