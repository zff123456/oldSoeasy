<template>
  <div class="right" v-loading="isLoading" element-loading-text="拼命加载中" style="padding-top:87px">
    ...
    <div class="djs" >
      ...
      <van-button    type="warning"  class="btns"  @click="stop"   style="width:20%;height:45px;position:absolute;right:23%;top:0;line-height:46px;color:#fff;text-align:center;box-sizing: border-box;" >暂停</van-button>
    ...
    <van-tabs title-active-color="#57B5E3" color="#57B5E3" :sticky="true" :offset-top="133">
      ...
      <van-tab title="听单句">
        ...
        <div v-for="(item, key, index) in tingdanju" class="duanwen-box">
          <div class="option-one" v-for="(ie, k, index) in item.xuanzhe[0]">
            <van-button icon="play-circle-o" type="primary" style="margin-left:.4rem" size="mini" @click="play_danjv(ie,key,k)">第({{k + 1}})小题听力</van-button>
            <el-popover
              placement="left"
              width="250"
              trigger="click">
      ...
      <van-tab title="词汇匹配">
        <div v-for="(item, key, index) in cihuipipei">
          ...
          <div class="xuanxiang" v-for="(ie, k, index) in item.xuanzhe">
            <div><div class="ckda" v-html="ie.wenti"></div></div>
            <van-cell-group>
      ...
      <van-tab title="英译汉">
        <div>
          <div><div class="ckda" v-html="yingyihan.wenzhang"></div></div>
          <van-cell-group>
...
export default {
  ...
  methods: {
    ...
    play_danjv(ie,key,k){ // 听单句
      this.$refs.xinwenAudio.src = `${this.baseUrl}${this.tingdanju[key].xuanzhe[0][k].audio_url}`;
