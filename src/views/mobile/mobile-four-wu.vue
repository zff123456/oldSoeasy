<template>
    <div class="right" v-loading="isLoading" element-loading-text="拼命加载中" style="padding-top:87px">
      ...
      <div class="djs" >
        ...
        <!-- <p style="width:33%;background:rgb(87, 181, 227);height:46px;position:absolute;font-size:0.5rem;right:0;top:0;line-height:46px;color:#fff;text-align:center" @click="open">交卷</p> -->
      ...
      <van-tabs title-active-color="#57B5E3" color="#57B5E3" :sticky="true" :offset-top="133">
        ...
        <van-tab title="听短文理解">
          ...
          <div v-for="(item, key, index) in duanwenlijie">
            ...
            <div v-for="(ie, k, index) in item.xuanzhe[0]">
              <van-button icon="play-circle-o" type="primary" style="margin-left:.4rem" size="mini" @click="play_chang_opt1(ie,key,k)">第({{k + 1}})小题听力</van-button>
              <el-popover
                placement="left"
                width="250"
                trigger="click">
          ...
          <div class="xctkdw" v-for="(item, key, index) in changyuedu.xuanzhe" style="margin-top:0;">
            <div class="xctkxx">
              <el-popover
                ...
                trigger="click">
                <template slot-scope="scope">
                  <div v-html="item.tiyan" style="border: 1px dashed #ccc;padding:10px;font-size: .32rem;max-height:70vh;overflow: auto;"></div>
                </template>
                <el-button slot="reference" type="warning" style="float:right;margin:10px" size="mini" v-show="is_tijiao">题眼</el-button>
              </el-popover>
        ...
        <van-tab title="篇章翻译">
          ...
          <div class="pzfytm">{{pianzhangfanyi.wenti}}</div>
          <van-cell-group>
  ...
  export default {
    ...
    methods: {
      ...
      play_chang_opt1(ie,key,k){ // 听长对话下的每一项的播放
        console.log(123)
