<template>
  <div class="rights">
    <div class="sea-box">
      ...
      <el-select v-model="type" placeholder="请选择试卷类型" @change="getUserList" style="width:150px">
        <el-option
    ...
    <div class="search-boxes">
      ...
      <el-select v-model="problem" placeholder="请选择题目类型" @change="getTimu" style="width:150px">
        <el-option
    ...
    <div class="content-box" v-if="problem == '作文' || problem == '英译汉' || problem == '篇章翻译'">
      ...
      <div style="width:110%;background-color: #F8F8F8;margin:60px 0 0;transform: translateX(-30px);"></div>
    </div>
    ...
    <div class="content-box" v-if="problem == '新闻报道' || problem == '听讲座对话'|| problem == '听长对话' || problem == '听短文理解' || problem == '听篇章' || problem == '听短对话'">
      ...
      <div style="margin:60px 0 0;transform: translateX(-30px);"></div>
    </div>
    <!-- 单独一种听单句 -->
...
export default {
  ...
  methods: {
    ...
    //   this.$refs.xinwenOpt[0].src = `${BASE_URL}${ie.audio_url}`;
    //   this.$refs.xinwenAudio.play();
    // },
    // play_xinwen(item){ // 新闻报道 duanwenAudio
    //   console.log(1)
    //   console.log(this.$refs)
    //   console.log(this.$refs.xinwenOpt)
    //   // console.log(this.$refs.xinwenOpt.src)
    //   this.$refs.xinwenOpt[0].src = `${BASE_URL}${item.audio_url}`;
    //   this.$refs.xinwenAudio.play();
    // },
    ...
    //  this.$refs.xinwenOpt[0].src = `${BASE_URL}${res[0].xuanzhe[0][k].audio_url}`;
    ...
    // },
     play_yuedu(ie,k){
      // console.log(ie,k);
     this.$refs.xinwenOpt[0].setAttribute("src",  `${BASE_URL}${ie.audio_url}`)
       this.$refs.xinwenOpt[0].play();
    },
    play_xinwen(item){ // 新闻报道 duanwenAudio
      // this.$refs.xinwenOpt[0].src = `${BASE_URL}${item.audio_url}`;
      this.$refs.xinwenOpt[0].setAttribute("src",  `${BASE_URL}${item.audio_url}`)
       this.$refs.xinwenOpt[0].play();
    },
    play_xinwen_opt(ie,k){ 
      const res = this.itemData.slice((this.currentPage -1) * this.pagesize, this.currentPage * this.pagesize);
      this.$refs.xinwenOpt[0].setAttribute("src", `${BASE_URL}${res[0].xuanzhe[0][k].audio_url}`)
      // this.$refs.xinwenOpt[0].src= `${BASE_URL}${res[0].xuanzhe[0][k].audio_url}`;
       this.$refs.xinwenOpt[0].play();
    },
  }
