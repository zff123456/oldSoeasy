...
export default {
  ...
  methods: {
    ...
    async getThesisList() {
      this.isLoading = true;
      const res = await this.$http.post("admin/paper/abcihuitiankonglst",qs.stringify({cet: this.paperId }));
    ...
    testPaperName(i){
      if(i == ''){return "不属于任何试卷"} else {return i};
    },
    tesPaperTypeName(i){
      if(i == ''){return "不属于任何题库"} else {return i};
    },
    revision(i,e){ // 编辑
