...
export default {
  ...
  methods: {
    ...
    async onSubmit() {
      if(this.wenzhang == ''){
        this.$message({message: "文章不能为空",type: "warning",duration: 2000});
        return false;
      }
      if(this.jiexi == ''){
        this.$message({message: "请填写解析",type: "warning",duration: 2000});
        return false;
      }
      if(this.cankaodaan == ''){
        this.$message({message: "请填写参考答案",type: "warning",duration: 2000});
        return false;
      }
