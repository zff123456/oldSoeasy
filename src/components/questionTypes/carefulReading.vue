  ...
  methods: {
    ...
    async onSubmit() {
      if(this.referenceVersion == ''){
        this.$message({message: "文章内容不能为空",type: "warning",duration: 2000});
        return false;
      }
      for ( let i = 0 ; i < this.options.length ; i++) {
        if(this.options[i].oneTranslate == ''){
          this.$message({message: "问题题目不能为空",type: "warning",duration: 2000});
          return false;
        }
        ...
        if(this.options[i].answer == ''){
          this.$message({message: "请选择正确答案",type: "warning",duration: 2000});
          return false;
        }
        if(this.options[i].tiyan == ''){
          this.$message({message: "问题题眼不能为空",type: "warning",duration: 2000});
          return false;
        }
        if(this.options[i].jiexi == ''){
          this.$message({message: "问题解析不能为空",type: "warning",duration: 2000});
          return false;
        }
      }
      const res = await this.$http.post("admin/paper/addCETzixiyuedu",qs.stringify({ 
        test_paper_id: this.paper_ty,
        wenzhang: this.referenceVersion,
